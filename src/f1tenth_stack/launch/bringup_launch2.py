...
def generate_launch_description():
    ...
    # Statyczny TF base_link -> laser_scan2 (20 cm bliżej base_link)
    static_tf_node_scan2 = Node(
        package='tf2_ros',
        executable='static_transform_publisher',
        name='static_baselink_to_laser_scan2',
        arguments=[
            '0.07', '0.0', '0.11',   # x y z (20 cm back from original 0.27)
            '0.0', '0.0', '0.0',     # roll pitch yaw
            'base_link', 'laser_scan2'
        ],
        output='screen'
    )
    ...
    scan_rotator_node = Node(
        package='f1tenth_stack',
        executable='scan_yaw_rotator',
        name='scan_yaw_rotator',
        output='screen',
        parameters=[{
            'input_topic': '/scan',
            'output_topic': '/scan_2',
            'output_frame_id': 'laser_scan2',
            'yaw_offset_deg': -220.0,
        }]
    )
