...
class StanleyAvoidance(Node):
    def __init__(self):
        ...
        self.declare_parameter("scan_topic", "/scan_2")
        self.declare_parameter("scan_yaw_offset_deg", 0.0)
        self.declare_parameter("scan_range_offset_m", 0.0)
        ...
        self.declare_parameter("avoidance_margin_m", 0.15)
        ...
        self.scan_yaw_offset_rad = math.radians(float(self.get_parameter("scan_yaw_offset_deg").value))
        self.scan_range_offset_m = float(self.get_parameter("scan_range_offset_m").value)
        ...
        self.avoidance_margin_m = float(self.get_parameter("avoidance_margin_m").value)
    ...
    def populate_occupancy_grid(self, ranges, angle_increment, angle_min):
        ...
        ranges = np.maximum(0.0, np.array(ranges) + self.scan_range_offset_m)
        indices = np.arange(len(ranges))
        thetas = angle_min + (indices * angle_increment) - self.ANGLE_OFFSET
    ...
    def scan_callback(self, scan_msg):
        ...
        angle_min = scan_msg.angle_min + self.scan_yaw_offset_rad
        self.populate_occupancy_grid(scan_msg.ranges, scan_msg.angle_increment, angle_min)
        ...
        MARGIN = int(self.CELLS_PER_METER * self.avoidance_margin_m)  # margin around the car in meters
